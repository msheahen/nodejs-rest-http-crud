...
try {
  ...
  const serviceHost = '172.30.76.236';
  const user = process.env.DB_USERNAME || process.env.POSTGRESQL_USER || 'mary';
  const password = process.env.DB_PASSWORD || process.env.POSTGRESQL_PASSWORD || 'password';
  const databaseName = process.env.POSTGRESQL_DATABASE || 'db';
